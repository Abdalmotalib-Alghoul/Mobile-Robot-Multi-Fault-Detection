#!/usr/bin/env python3
"""
ENHANCED Dataset Collector for LiDAR Crosstalk Detection Training
Updated for crosstalk injector compatibility - RAW-ONLY VERSION
"""
...
class EnhancedDatasetCollector:
    def __init__(self):
        rospy.init_node('enhanced_dataset_collector', anonymous=True)
        
        # Parameters - KEEP SIMPLE
        self.experiment_duration = float(rospy.get_param('~experiment_duration', 60.0))
        self.scenario_name = rospy.get_param('~scenario_name', 'crosstalk_injection')
        self.run_id = rospy.get_param('~run_id', '1')
        self.logging_frequency = float(rospy.get_param('~logging_frequency', 10.0))
        
        # Data collection
        self.save_directory = rospy.get_param('~injector_save_dir', "/home/talib/catkin_ws/Plot/outlier_analysis")
        ...
        rospy.loginfo(f"ENHANCED Collector - Crosstalk Detection Dataset (Raw-Only)")
    ...
    def setup_subscribers(self):
        ...
        try:
            ...
            rospy.loginfo("Enhanced subscribers setup - Crosstalk compatible")
    ...
    def log_data(self):
        ...
        with self.data_lock:
            ...
            # Assign all current faults as columns
            for key, value in self.current_faults.items():
                row[key] = value
            
            # Explicitly set legacy labels
            row['has_outlier'] = 0  # Outlier is a distinct fault, not crosstalk
            row['has_dropout'] = 0
            row['has_noise'] = 0
            row['has_drift'] = 0
        ...
        # Log crosstalk rate every 50 rows
        if len(self.dataset) % 50 == 0 and len(self.dataset) > 0:
            crosstalk_rate = sum(d['has_crosstalk'] for d in self.dataset) / len(self.dataset)
            rospy.loginfo(f"Rows: {len(self.dataset)}, Crosstalk rate: {crosstalk_rate:.2%}")

    def save_data(self):
        """Save raw dataset with crosstalk labels"""
        try:
            if not self.dataset:
            ...
            # Data validation
            lidar_cols = [col for col in df.columns if col.startswith('lidar_beam_')]
            nan_pct = df[lidar_cols].isnull().sum(axis=1) / len(lidar_cols)
            df = df[nan_pct < 0.1]
            df[lidar_cols] = df[lidar_cols].clip(0.15, 12.0)
            
            if df.isnull().any().any():
                rospy.logwarn("Dataset contains NaN values - filling with zeros")
                df = df.fillna(0)
            
            filename = f"crosstalk_dataset_{self.scenario_name}_run{self.run_id}_{self.timestamp}.csv"
            ...
            crosstalk_rows = df[df['has_crosstalk'] == 1]
            normal_rows = df[df['has_crosstalk'] == 0]
            ...
            rospy.loginfo(f"✅ SAVED CROSSTALK DATASET: {filename}")
            rospy.loginfo(f"   Total Rows: {len(df)}")
            rospy.loginfo(f"   Crosstalk Samples: {len(crosstalk_rows)} ({len(crosstalk_rows)/len(df)*100:.1f}%)")
    ...
    def shutdown_handler(self):
        ...
        crosstalk_count = sum(d['has_crosstalk'] for d in self.dataset)
        rospy.loginfo(f"Shutdown - Collected {len(self.dataset)} rows, {crosstalk_count} crosstalk samples in {elapsed_time:.1f}s")
        self.save_data()
    ...
    def run(self):
        ...
        self.start_time = time.time()
        rospy.loginfo(f"Starting crosstalk data collection at {self.logging_frequency} Hz")
