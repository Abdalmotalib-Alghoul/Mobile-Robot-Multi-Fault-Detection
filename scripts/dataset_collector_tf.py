...
class UnifiedFaultCollector:
    ...
    def setup_subscribers(self):
        ...
        rospy.Subscriber('/tf_anomalous', TFMessage, self.tf_anomalous_callback)
    ...
    def tf_anomalous_callback(self, msg):
        with self.data_lock:
            self.latest_tf_anomalous = self.extract_odom_base_tf(msg)
            if self.latest_tf_anomalous:
                self.last_stamps['tf_anomalous'] = msg.transforms[0].header.stamp if msg.transforms else rospy.Time.now()

    def extract_odom_base_tf(self, msg):
    ...
    def is_data_ready(self):
        ...
        if not ready:
            rospy.logdebug_throttle(20, "Waiting for core data (imu, odom, scan)")
        return ready
    ...
    def get_sensor_ranges(self):
        ...
        return np.full(self.beam_count, 12.1)

    def get_tf_pose(self):
        now = rospy.Time.now()
        if self.latest_tf_anomalous and (now - self.last_stamps.get('tf_anomalous', rospy.Time(0))).to_sec() <= 0.5:
            return self.latest_tf_anomalous
    ...
    def downsample_ranges(self, ranges):
        ranges = np.where(np.isinf(ranges), 12.1, ranges)
        ranges = np.nan_to_num(ranges, nan=12.1)
        if len(ranges) <= self.beam_count:
            padded = np.full(self.beam_count, 12.1)
    ...
    def compute_particle_stats(self):
        ...
        try:
            ...
            return {
                ...
                'particle_mean_yaw': float(np.mean(yaws)),
                'particle_std_x': float(np.std(xs)),
                'particle_std_y': float(np.std(ys)),
                'particle_std_yaw': float(np.std(yaws))
            }
        except Exception as e:
    ...
    def log_data(self):
        ...
        with self.data_lock:
            ...
            # TF pose + yaw (only anomalous when active, else default)
    ...
    def intermediate_save(self):
        current_time = time.time()
        if current_time - self.last_intermediate_save > 120.0:
