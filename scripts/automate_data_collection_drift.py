...
def parse_args():
    parser = argparse.ArgumentParser(description="Automate data collection for LiDAR drift experiments.")
    parser.add_argument("--ros_workspace", type=str, default="/home/talib/catkin_ws", help="Path to the ROS workspace.")
    parser.add_argument("--launch_file_path", type=str, default="/home/talib/catkin_ws/src/stretch_ros/stretch_navigation/launch/navigation_gazebo_drift.launch", help="Path to the roslaunch file.")
    parser.add_argument("--save_base_dir", type=str, default="/home/talib/collected_datasets_drift", help="Base directory to save collected datasets.")
    parser.add_argument("--injector_save_dir", type=str, default="/home/talib/catkin_ws/Plot/drift_analysis", help="Directory where dataset_collector_drift saves its CSVs.")
    parser.add_argument("--scenarios_file", type=str, default="/home/talib/catkin_ws/src/stretch_ros/stretch_navigation/config/scenarios_drift.yaml", help="Path to scenarios YAML configuration file.")
...
def force_kill_ros_gazebo():
    ...
    kill_patterns = [
        ...
        'dataset_collector_drift', 'enhanced_lidar_drift_injector', 'launch_terminator',
...
def collect_and_move_files(scenario_name, run_id, injector_save_dir, save_base_dir):
    ...
    patterns.insert(0, f"drift_dataset_{scenario_name}_run{run_id}*.csv")
    ...
    for pattern in patterns:
        ...
        if matching_files:
            ...
            if os.path.getsize(found_file) > 100:  # At least 100 bytes
                found_files = {"dataset_drift": found_file}
                print(f"Found data file for {scenario_name} (Run {run_id}): {os.path.basename(found_file)}")
                
                # Move to save directory
                new_filename = f"dataset_drift_{scenario_name}_run{run_id}_{current_timestamp}.csv"
