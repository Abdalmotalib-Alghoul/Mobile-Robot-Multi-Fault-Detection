#!/usr/bin/env python3
"""
Remove one feature from each high-correlation pair (|r| >= 0.85)
Saves cleaned dataset with reduced features
2026-03 CoDIT compatible - minimal & reproducible

Strategy: for each pair (highest corr first), keep the feature
          whose name is later in alphabetical order → deterministic
"""
...
def main():
    ...
    for _, row in high_pairs.iterrows():
        ...
        # Already decided to remove one → skip
        if f1 in to_remove or f2 in to_remove:
            continue

        # Deterministic choice: keep the one later in alphabet
        if f1 < f2:
            remove = f1
            keep  = f2
        else:
