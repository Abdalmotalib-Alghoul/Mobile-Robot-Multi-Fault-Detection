"""
- All features computed exclusively inside each window 
...
# ────────────────────────────────────────────────
# EXPERT FEATURE EXTRACTION FUNCTION 
# ────────────────────────────────────────────────
...
def extract_expert_features(w_beams, w_tf_x, w_tf_y, w_tf_yaw,
    ...
    # ===== INTEGRATED FEATURES (7 features) =====
    ...
    # ===== RUN-INVARIANT FLAGS (3 features) =====
...
# ────────────────────────────────────────────────
# TRULY NEW: Dedicated slow constant bias detectors 
# ────────────────────────────────────────────────
...
# 1. Forward-beam weighted bias
    ...
    features.append(float(forward_bias))                    
...
# 4. Innovation consistency ratio 
...
for run_id, group in df.groupby('run_id'):
    ...
    for i in range(WINDOW_SIZE - 1, n, STEP_SIZE):
        ...
        # Feature names from the second script 
